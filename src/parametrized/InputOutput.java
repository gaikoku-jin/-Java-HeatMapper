...
public class InputOutput {
	...
	public static void save (int iks, double[][] xMesh, double[][] yMesh, double[][] y_negMesh, double[][] zMesh, double[][] TMesh, int[] nodes, int ndp, int time) {
		
	//	double moisture = (double)Math.round((1-iks/10.0) * 10) / 10 ;
	...
	public static int[] param (File plik) {
		
		int[]parameters = new int[15];
		...
		try (Scanner sc = new Scanner(plik)){
			...
			while (sc.hasNextLine()) {
				String linia = sc.nextLine();
					...
					parameters[m]=Integer.parseInt(linia);
					m++;
			}
